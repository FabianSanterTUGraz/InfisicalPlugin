...
public class InjectSecretsRunConfigurationExtensionTest extends BasePlatformTestCase {
    ...
    // Spring-Boot-Configs werden seit dem Split auf InjectSecretsRunConfigurationExtensionSpringBoot
    // NICHT mehr von dieser (Gradle-only) Extension abgedeckt - siehe
    // InjectSecretsRunConfigurationExtensionSpringBootTest fuer das Pendant.
    public void testIsApplicableFor_springBootConfiguration_isFalse() {
        assertFalse(extension.isApplicableFor(createSpringBootConfiguration()));
    }
    ...
    // Nur noch fuer den testIsApplicableFor_springBootConfiguration_isFalse-Fall benoetigt - die
    // eigentliche Spring-Boot-Abdeckung liegt jetzt in InjectSecretsRunConfigurationExtensionSpringBootTest.
    private RunConfigurationBase<?> createSpringBootConfiguration() {
        ConfigurationFactory factory = com.intellij.spring.boot.run.SpringBootApplicationConfigurationTypeBase.getInstance().getDefaultConfigurationFactory();
        return (RunConfigurationBase<?>) factory.createTemplateConfiguration(getProject());
    }
    ...
    public static String fakeJwt(long expiresInSeconds) {
